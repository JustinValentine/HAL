...
public:
    // Constructor that takes the path to the serial port (e.g., "/dev/ttyUSB0")
    UM7(const char* serial_path);

	// Destructor to close the serial port
    ~UM7();

	// Method to write data to the serial port
    void writeToSerial(const uint8_t* buffer, size_t length);
	...
	bool decode(uint8_t current_byte);
...
private:
    ...
    int state;           // Your state variable
    struct termios tty;  // termios structure to store serial port settings
