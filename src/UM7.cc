...
// Constructor 
UM7::UM7(const char* serial_path) {
	serial_port.Open(serial_path);
	serial_stream.Open(serial_path);
    ...
    if (serial_port.IsOpen()) {
        cout << "SUCCESSFUL: serial port opened at: " << serial_path << endl;
        usleep(5000);
    } else {
        cerr << "ERROR: Could not open serial port at: " << serial_path << endl;
        throw std::runtime_error("Failed to open serial port");
    }
}

void send_config_buffer(SerialPort& serial_port, SerialStream& serial_stream, uint8_t* config_buffer, size_t length) {
    std::vector<uint8_t> data_buffer(config_buffer, config_buffer + length);

    // serial_port.Write(data_buffer);
	serial_stream.write( reinterpret_cast<const char*>(config_buffer), length);
}

void send_cmd_buffer(SerialPort& serial_port, SerialStream& serial_stream, uint8_t* cmd_buffer, size_t length) {
    std::vector<uint8_t> data_buffer(cmd_buffer, cmd_buffer + length);

    // serial_port.Write(data_buffer);
	serial_stream.write( reinterpret_cast<const char*>(cmd_buffer), length);
}
...
*/
bool UM7::decode(uint8_t current_byte) {
...
void UM7::set_sensor_baud_rate(float baud) {
	...
	send_config_buffer(serial_port, serial_stream, config_buffer, 11);
}
...
void UM7::set_sensor_baud_rate(float baud, float gps_baud, bool gps, bool sat) {
	uint8_t rate = 0, gps_rate = 0, b1 = 0, b0 = 0;
...
char* UM7::get_firmware_revision() {
	...
	send_config_buffer(serial_port, serial_stream, cmd_buffer, 7);
	// serial_port->read(firmware, 4);
...
void UM7::factory_reset() {
	...
	send_config_buffer(serial_port, serial_stream, cmd_buffer, 7);
    std::cout << "FACTORY RESET UM7" << std::endl;}
